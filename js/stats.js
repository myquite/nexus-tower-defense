...
const SRC = {
  ...
  METEOR: 5, NOVA: 6, DETONATE: 7, THORNS: 8,
  BARRIER: 9, FISSION: 10, OTHER: 11,
};
...
const SRC_META = [
  ...
  // Both of these are damage the player never aimed: the barrier burns what
  // walks into it, and a Fission crit sets off its own blast. They are split
  // from their triggers on purpose — folding Fission into CORE GUN would hide
  // exactly the thing the player bought it to find out.
  { name: 'ARC BARRIER',  icon: '✴️',  color: C.cyan },
  { name: 'FISSION',      icon: '☢',   color: C.orange },
...
const Stats = {
  ...
  reset(g) {
    ...
    // The barrier is the only defence that is spent rather than worn down, so
